    ...
    if (cgi == 1) 
    {
        ...
        else if (m_method == POST && strncasecmp(p+1, "v1/chat/completions", 19) == 0)
        {
            ...
            while(true){
                ...
                std::cout << m_redis_pool->GetFreeConn() <<std::endl;
                // 这里再获取一个redis
                std::cout << "获取redis1" <<std::endl;
                redis_connectionRAII redislcon(&redis, m_redis_pool);
                std::cout << "获取redis2" <<std::endl;
                std::cout << m_redis_pool->GetFreeConn() <<std::endl;
                dcr_apikey(redis, m_apikey);
                m_lock.unlock();
                break;
            ...
            // bool is_write = write_file(content, filename);
            // std::cout << is_write << std::endl;
            // kimi_upload_file(filename, content, content_type);
            std::string uuid = gen_uuid();
            std::string suffix = splitString(filename, '.').back();
            bool is_write = write_file(content, uuid + "." + suffix);

            // 构建发送到rabbitmq的消息
            string queue = "file_parse";
            std::string task_str1 = R"(
                "task_id" : "{}",
                "suffix": "{}"
            )";
            string task_str = '{' + fmt::format(task_str1, uuid, suffix) + '}';
            nlohmann::json task_str_j = nlohmann::json::parse(task_str);
            // 将 JSON 对象转换回字符串
            std::string task_json_str = task_str_j.dump();
            // 去一个rabbitmq连接
            rabbitmq_connectionRAII rabbitmqlcon(&rabbitmq, m_rabbitmq_pool);
            // AmqpClient::Channel::ptr_t channel = AmqpClient::Channel::Create("1.92.101.211", 5672, "admin", "admin");
            rabbitmq->DeclareQueue(queue, false, false, false, false);
            
            rabbitmq->BasicPublish("", queue, AmqpClient::BasicMessage::Create(task_json_str));
            
            // parseFormData(receivedData, boundary); 
            // return NO_REQUEST;
            // char* m_response_str1 = R"({
            //     "id": "cv4n4v4qdqei91n69080",
            //     "object": "file",
            //     "bytes": 74180,
            //     "created_at": 1741255292,
            //     "filename": "屏幕截图 2024-07-10 205207.png",
            //     "purpose": "file-extract",
            //     "status": "ok",
            //     "status_details": ""
            // }  
            // )";
            std::string m_response_str1 = R"(
                "id": "{}",
                "object": "file",
                "bytes": {},
                "created_at": 1741255292,
            ...
            // std::string m_response_str2 = fmt::format("\"id\": \"{}\", 
            //     \"object\": \"file\",
            //     \"bytes\": {},
            //     \"created_at\": \"1741255292\", 
            //     \"filename\": \"{}\",
            //     \"purpose\": \"file-extract\",
            //     \"status\": \"ok\",
            //     \"status_details\": \"\"", uuid, content.size(), filename);
            std::string m_response_str2 = '{' + fmt::format(m_response_str1, uuid, content.size(), filename) + '}';
            // 解析 JSON 字符串
            nlohmann::json j = nlohmann::json::parse(m_response_str2);
