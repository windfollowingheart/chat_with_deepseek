...
http_conn::HTTP_CODE http_conn::do_request() 
{
    ...
    if (cgi == 1) 
    {
        ...
        //     std::string _password = request_json["password"].get<std::string>();
        //     strcpy(name, email_or_phone.c_str());
        //     strcpy(password, _password.c_str());
        //     name[email_or_phone.size()] = '\0';
        ...
        if (m_method == POST && strncasecmp(p+1, "register", 8) == 0)
        {
            ...
            if(!json1.contains("isok")){ // 前面没有出错才继续执行后面逻辑
                //如果是注册，先检测数据库中是否有重名的
                //没有重名的，进行增加数据
        ...
        else if (m_method == POST && strncasecmp(p+1, "login", 5) == 0)
        {
            ...
            if (users.find(email_or_phone.c_str()) != users.end() && users[email_or_phone] == password)
            {
                ...
                m_response_str = new char[r_json_str.length() + 1];
                std::strcpy(m_response_str, r_json_str.c_str());
                // std::cout << m_response_str <<std::endl;
                return JSON_REQUEST;
            }
        }
        ...
        else if (m_method == POST && strncasecmp(p+1, "v1/chat/completions", 19) == 0)
        {
            ...
            bool search = false;
            std::cout << user_id << msg_idx << idx << std::endl;
            ...
            if(is_new){
                ...
                std::cout << "插入数据库状态 chats " << res << std::endl;
            }else{
                std::cout << chat_id << create_time << std::endl;
                ...
                std::cout << "更新数据库状态 chats " << res << std::endl;
            }
            ...
            if(is_replay){
                ...
                std::cout << "更新数据库状态 messages " << res << std::endl;
            }else {
                ...
                int res = mysql_query(mysql, sql_insert.c_str());
                std::cout << sql_insert << std::endl;
                std::cout << "插入数据库状态 messages " << res << std::endl;
            }
            m_lock.unlock();
            ...
            int res = mysql_query(mysql, sql_update.c_str());
            m_lock.unlock();
            std::cout << "更新数据库状态 chats chat_name" << res << std::endl;
            ...
            m_lock.unlock();
            std::cout << "删除数据库状态 chats " << res << std::endl;
    ...
    else if (m_method == GET && strncasecmp(p+1, "historys", 8) == 0)
    {
        ...
        std::cout << sql_select_userid << std::endl;
