...
// 生成表单数据
std::string buildFormData(const std::map<std::string, std::string>& formDataMap, const std::string& boundary) {
    std::string formData;
...
nlohmann::json get_josn_body(std::string &all){
...
// 发送 HTTPS 表单请求
void sendHttpsFormRequest(const std::string& serverUrl, const std::string& serverIp, int serverPort, const std::map<std::string, std::string>& formDataMap) {
    ...
    int status = getaddrinfo(serverIp.c_str(), "443", &hints, &res);
    ...
    // 构建表单数据
    std::string formData = buildFormData(formDataMap, boundary);
    ...
    nlohmann::json json_body = get_josn_body(aa);
...
void kimi_upload_file(const std::string &filename, const std::string &fileContent, const std::string &fileContentType) {
    ...
    std::string serverUrl = "/v1/files";
    std::string serverIp = "api.moonshot.cn";
    int serverPort = 443;
    sendHttpsFormRequest(serverUrl, serverIp, serverPort, formDataMap);
}