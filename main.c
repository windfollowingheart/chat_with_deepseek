...
#include "./utils/utils/token.h"


#define MAX_FD 65536           //最大文件描述符
#define MAX_EVENT_NUMBER 10000 //最大事件数
#define TIMESLOT 50000            //最小超时单位
...
int main(int argc, char *argv[])
{
    ...
    redis_connPool->init("127.0.0.1", "", "", 0, 6379, 3);

    // 创建rabbitmq连接池
    rabbitmq_connection_pool *rabbitmq_connPool = rabbitmq_connection_pool::GetInstance();
    rabbitmq_connPool->init("1.92.101.211", "admin", "admin", 5672, 3);

    // 创建文件上传python后端连接池
    uploadfile_connection_pool *uploadfile_connPool = uploadfile_connection_pool::GetInstance();
    uploadfile_connPool->init(3);

    // 创建获取解析结果python后端连接池
    getparseresult_connection_pool *getparseresult_connPool = getparseresult_connection_pool::GetInstance();
    ...
    try
    {
        pool = new threadpool<http_conn>(connPool, redis_connPool, rabbitmq_connPool, 
            uploadfile_connPool, getparseresult_connPool,
            &tokenPool);
    }
    catch (...)
    {
    ...
    //初始化reids数据库apikeys
    users->initredis_result(redis_connPool);

    // // 设置token
    // users->m_token_pool = &tokenPool;
