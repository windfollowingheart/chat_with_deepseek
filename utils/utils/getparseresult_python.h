#ifndef _GET_PARSE_RESULT_PYTHON_
#define _GET_PARSE_RESULT_PYTHON_
...
std::string getparseresult_buildFormData(const std::map<std::string, std::string>& formDataMap, const std::string& boundary) {
    std::string formData;
...
// 解析 JSON 数据
template<typename T = void>
nlohmann::json getparseresult_get_josn_body(std::string &all){
...
// 发送 HTTPS 表单请求
template<typename T = void>
nlohmann::json getparseresult_sendHttpsFormRequest(const std::string& serverUrl, const std::string& serverIp, int serverPort) {
    ...
    // 构建表单数据
    // std::string formData = getparseresult_buildFormData(formDataMap, boundary);

    // 构建 HTTP 请求头
    std::string request = "GET " + serverUrl + " HTTP/1.1\r\n";
    ...
    nlohmann::json json_body = getparseresult_get_josn_body(aa);
    if(json_body == NULL){
...
// 上传文件到 Python 服务器
template<typename T = void>
nlohmann::json getparsereuslt_from_pythonsever(const std::string &task_id) {
    ...
    // };
    std::string serverUrl = "/file/parse/" + task_id;
    std::string serverIp = "127.0.0.1";
    int serverPort = 8000;
    return getparseresult_sendHttpsFormRequest(serverUrl, serverIp, serverPort);
}
